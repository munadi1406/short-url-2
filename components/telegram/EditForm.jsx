...
export default function EditForm({ data }) {
    ...
    const [updateData, setUpdateData] = useState({ title: '', id_chanel: '', id: '' })
    ...
    useEffect(() => {
        const isEdited =
            ...
            updateData.id_chanel !== data.id_chanel ||
    ...
    const { mutate, isPending, isSuccess } = useMutation({
        mutationFn: async (e) => {
            e.preventDefault()
            setMsg("")
            const updateChanel = await axios.put('/api/telegram', updateData)
            return updateChanel.data
        },
        onSuccess: (data) => {
            setMsg(data.msg)
        ...
        <form className="border-l-2 border-blue-600 p-2 flex flex-col gap-1" onSubmit={mutate} method="post">
            ...
            <div className="grid md:grid-cols-2 gap-2 grid-cols-1">
                ...
                <div>
                    ...
                    <Input id="editLink" name="id_chanel" value={updateData.id_chanel} onChange={handleChange} />
                </div>
            </div>
            <div>
