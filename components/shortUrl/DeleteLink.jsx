...
export default function DeleteLink({ data,refetch }) {
    ...
    const {mutate,isPending} = useMutation({
        mutationFn: async (formData) => {
            ...
            const updateLink = await axios.delete(`/api/shorten?id=${formData.id}`)
    ...
    return (
        <Button variant="destructive" onClick={()=>mutate(data)} disabled={isPending} type="button">Hapus</Button>
    )
}
